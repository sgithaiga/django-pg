...
from .models import Post, Vacancies, Applications, HrDocs, CorpDocs, CompanyApps, FAQs
...
#vacancies descriptions in detail view
...
#hr documents listing in class list view
class HrdocListView(ListView):
 ...
 #corporate documents in class list view
class Corp_docListView(ListView):
    model = CorpDocs
    template_name = 'blog/Corp_doc.html' #<app>/<model>_<viewtype>.html
    context_object_name = 'posts'
    ordering = ['-date_uploaded']
    paginate_by = 5   

#company apps page in class list view
class AppsListView(ListView):
    model = CompanyApps
    template_name = 'blog/apps.html' #<app>/<model>_<viewtype>.html
    context_object_name = 'posts'
    paginate_by = 5
...
class FAQsListView(ListView):
    model = CompanyApps
    template_name = 'blog/faqs.html' #<app>/<model>_<viewtype>.html
    context_object_name = 'posts'
    ordering = ['-date_uploaded']
    paginate_by = 5
